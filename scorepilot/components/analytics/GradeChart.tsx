...
function ChartTooltip({
  ...
  const subjectEntries = payload.filter(
    (entry) => entry.name !== "전체 평균" && entry.value != null,
  );
  const avg = payload.find((entry) => entry.name === "전체 평균");

  return (
    <div className="min-w-[160px] rounded-lg border bg-white px-3 py-2.5 text-sm shadow-md">
